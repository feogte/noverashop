...
async def claim_gift_account(call: CallbackQuery):
    ...
    phone_number = prod["phone"] or "Номер не указан"
...
async def gift_acc_session(m: Message, state: FSMContext):
    ...
    await m.answer("🎁 <b>3 этап</b> — Введите <b>номер телефона</b> аккаунта (например +1 555-1234 или +79991112233):")
...
async def gift_confirm_send(call: CallbackQuery, state: FSMContext):
    ...
    conn = await db()
    cur = await conn.execute(
        "INSERT INTO products(kind, name, price_rub, price_stars, price_kzt, stock, sales, created_at, file_id, session_path, phone) VALUES('account', ?, 0, 0, 0, 0, 1, ?, ?, ?, ?)",
        (phone, now_msk().isoformat(), file_id, session_path, phone)
    )
    ...
    try:
        ...
        await call.message.answer(f"⚠️ Не удалось доставить сообщение пользователю в ЛС (возможно, заблокировал бота): {e}")
...
async def stock(m):
    ...
    kb.adjust(1)
    await m.answer("Управление складом (выберите товар для настройки):", reply_markup=kb.as_markup())
...
async def manage_product_menu(call: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🔢 Изменить количество", callback_data=f"edit_stock:{pid}")],
...
async def delete_product_handler(call: CallbackQuery):
    ...
    await conn.commit(); await conn.close()
    await call.answer("Товар успешно удален!", show_alert=True)
    await stock(call.message)
...
@dp.callback_query(F.data.startswith("edit_stock:"))
async def edit_stock_start(call: CallbackQuery, state: FSMContext):
    pid = int(call.data.split(":")[1])
    await state.update_data(edit_pid=pid)
    await state.set_state(EditProduct.stock)
    await call.message.answer("Введите новое количество товара:")
    await call.answer()

@dp.message(EditProduct.stock)
async def edit_stock_finish(m: Message, state: FSMContext):
    data = await state.get_data()
    try: val = int(m.text)
    except ValueError: return await m.answer("Введите целое число.")
    conn = await db()
    await conn.execute("UPDATE products SET stock=? WHERE id=?", (val, data["edit_pid"]))
    await conn.commit(); await conn.close()
    await state.clear()
    await m.answer("Количество товара обновлено! ✅", reply_markup=admin_kb())
...
async def add_account_file(m: Message, state: FSMContext):
    ...
    await state.update_data(file_id=file_id, session_path=str(save_path))
    await state.set_state(AddProduct.phone)
    await m.answer("6 этап — Введите номер телефона (например +79991112233):")
