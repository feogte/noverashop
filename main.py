...
def admin_kb():
    kb = ReplyKeyboardBuilder()
    kb.button(text="Статистика"); kb.button(text="Склад")
    kb.button(text="Рассылка"); kb.button(text="Изменить текст")
...
async def add_stars(m: Message, state: FSMContext):
    ...
    if d.get("kind") == "account":
        ...
        await m.answer("Введите количество товара в наличии:")
...
async def add_account_file(m: Message, state: FSMContext):
    ...
    await state.set_state(AddProduct.phone)
    await m.answer("5 этап — Введите номер телефона этого аккаунта (например +79991112233):")
...
# --- ПОЛЬЗОВАТЕЛЬСКОЕ МЕНЮ (ТОЛЬКО ВНЕ СОСТОЯНИЙ) ---
...
async def claim_account(call: CallbackQuery):
    ...
    await call.message.answer(
        ...
        f"Отправьте запрос кода авторизации в приложении. Как только код придет, бот перешлит его сюда."
    )
