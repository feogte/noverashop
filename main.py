...
# РЕКВИЗИТЫ ДЛЯ ОПЛАТЫ КАРТОЙ / СБП
...
FLAGS = {"1":"🇺🇸","7":"🇷🇺","20":"🇪🇬","27":"🇿🇦","30":"🇬🇷","31":"🇳🇱","32":"🇧🇪","33":"🇫🇷","34":"🇪🇸","36":"🇭🇺","39":"🇮🇹","40":"🇷🇴","41":"🇨🇭","43":"🇦🇹","44":"🇬🇧","45":"🇩🇰","46":"🇸🇪","47":"🇳🇴","48":"🇵🇱","49":"🇩🇪","51":"🇵🇪","52":"🇲🇽","53":"🇨🇺","54":"🇦🇷","55":"🇧🇷","56":"🇨🇱","57":"🇨🇴","58":"🇻🇪","60":"🇲🇾","61":"🇦🇺","62":"🇮🇩","63":"🇵🇭","64":"🇳🇿","65":"🇸🇬","66":"🇹🇭","81":"🇯🇵","82":"🇰🇷","84":"🇻🇳","86":"🇨🇳","90":"🇹🇷","91":"🇮🇳","92":"🇵🇰","93":"🇦🇫","94":"🇱🇰","95":"🇲🇲","98":"🇮🇷","212":"🇲🇦","213":"🇩🇿","216":"🇹🇳","218":"🇱🇾","234":"🇳🇬","254":"🇰🇪","255":"🇹ℤ","380":"🇺🇦","381":"🇷🇸","420":"🇨🇿","421":"🇸🇰","423":"🇱🇮","852":"🇭🇰","853":"🇲🇴","886":"🇹🇼","972":"🇮🇱","971":"🇦🇪","995":"🇬🇪","998":"🇺🇿"}
...
async def update_user_count_description():
    try:
        conn=await db(); count=(await one(conn,"SELECT COUNT(*) c FROM users"))["c"]; await conn.close(); await bot.set_my_short_description(short_description=f"👥 Пользователей: {count}")
    except Exception: pass
async def save_user(m):
    u=m.from_user; conn=await db(); await conn.execute("INSERT INTO users(id,username,first_name,created_at) VALUES(?,?,?,?) ON CONFLICT(id) DO UPDATE SET username=excluded.username,first_name=excluded.first_name",(u.id,u.username,u.first_name,now_msk().isoformat())); await conn.commit(); await conn.close(); await update_user_count_description()
async def save_user_id(user):
    conn=await db(); await conn.execute("INSERT INTO users(id,username,first_name,created_at) VALUES(?,?,?,?) ON CONFLICT(id) DO UPDATE SET username=excluded.username,first_name=excluded.first_name",(user.id,user.username,user.first_name,now_msk().isoformat())); await conn.commit(); await conn.close(); await update_user_count_description()
...
async def is_subscribed(user_id):
    ...
    except Exception as e: print(f"[SUBSCRIPTION CHECK ERROR] user={user_id}: {type(e).__name__}: {e}"); return False
...
async def add_kind(call, state):
    if not await subscription_gate(call):
        return
...
async def add_rub(m, state):
    ...
    try:
        ...
        return await m.answer("Введите число, например 50")
...
async def add_stars(m, state):
    ...
    try:
        ...
        return await m.answer("Введите целое число, например 60")
...
async def add_stock(m, state):
    ...
    if count < 0:
        return await m.answer("Количество не может быть отрицательным.")
...
async def add_session(m, state):
    ...
    await m.answer("Файл получен! Теперь введите номер телефона аккаунта (например: +79991112233):")

@dp.message(AddProduct.session_file)
async def add_session_wrong(m):
    await m.answer("Пожалуйста, отправьте именно `.session` файл как документ.")

@dp.message(AddProduct.phone_number)
async def add_phone(m, state):
    if not await subscription_gate(m) or not is_admin(m.from_user.id):
        return
    phone = m.text.strip()
    d = await state.get_data()
...
async def listen_for_telegram_code(session_path: str, user_id: int):
    client = Client("temp_session", api_id=API_ID, api_hash=API_HASH, session_string=session_path) if not session_path.endswith(".session") else Client(session_path.replace(".session",""), api_id=API_ID, api_hash=API_HASH)
    try:
...
async def purchase_action(call):
    ...
    if action=="approve":
        ...
        try:
            msg_text = f"✅ Ваша покупка #{pid} одобрена!\n\n"
            if prod and prod["phone_number"]:
                ...
                await bot.send_message(p["user_id"], msg_text)
