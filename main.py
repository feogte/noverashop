...
# --- ВЫДАЧА И PYROGRAM СЛУШАТЕЛЬ С АВТО-ОПРЕДЕЛЕНИЕМ НОМЕРА ---
...
# --- ДОБАВЛЕНИЕ ТОВАРА (С АВТО-ОБЪЕДИНЕНИЕМ И БЕЗ ЗАПРОСА НОМЕРА) ---
...
async def add_account_file(m: Message, state: FSMContext):
    ...
    conn = await db()
    existing = await one(conn, "SELECT id, stock FROM products WHERE kind=? AND name=?", (kind, product_name))
    
    if existing:
        await conn.execute(
            "UPDATE products SET stock = stock + 1, price_rub=?, price_stars=?, price_kzt=?, session_path=?, file_id=? WHERE id=?",
            (d["price_rub"], d["price_stars"], d["price_kzt"], str(save_path), file_id, existing["id"])
        )
        msg_text = f"✅ <b>Количество товара «{product_name}» увеличено!</b>\nТеперь на складе: <b>{existing['stock'] + 1} шт.</b>"
    else:
        await conn.execute(
