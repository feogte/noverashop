...
async def safe_listen_for_login_code(session_path: str, user, phone_number: str):
    ...
    try:
        ...
        async def code_handler(cli, message):
            ...
            # 1. Отправляем код авторизации покупателю
            ...
            # 2. При подтверждении входа Telegram пишем админу
...
async def broadcast_finish(m: Message, state: FSMContext):
    ...
    # Берем точный ID сообщения, которое прислал админ для рассылки
    ...
    for u in users:
        try:
            ...
            count += 1
            await asyncio.sleep(0.05)  # Задержка 50мс против FloodWait Telegram
        except Exception as e:
