...
# Проверка, содержится ли уже флаг (эмодзи) в тексте
def has_flag_emoji(text: str) -> bool:
    if not text:
        return False
    # Диапазон юникода для флагов-эмодзи (Regional Indicator Symbols)
...
async def init_db():
    ...
    """)
    # Авто-миграция для существующей БД (добавление столбца price_kzt при необходимости)
    try:
...
# Новая схема состояний добавления товара (6 этапов)
...
async def products(m: Message, kind: str):
    ...
    for p in rows: 
        # Проверка флага: если ручной флаг в названии уже есть, автофлаг не подставляется
...
async def select_product(call: CallbackQuery):
    ...
    # Кнопки для выбора оплаты (3 варианта)
    kb = InlineKeyboardMarkup(inline_keyboard=[
