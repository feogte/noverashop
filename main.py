...
# Pyrogram API для слушателя и работы с вашим аккаунтом
API_ID = 1234567
...
def check_antifraud(user_id: int) -> bool:
    ...
    Здесь проверяется структура рефералов в БД.
    """
    conn = get_db()
    cursor = conn.cursor()
    ...
    # Простейшая проверка: если рефералы регистрировались подозрительной пачкой по алгоритму
    # В реальном сетапе можно делать расширенную проверку аккаунтов
    if len(refs) < 30:
        return False
    return True
...
async def cmd_start(message: types.Message):
    ...
    # 1. Запись в БД считывает всех новых пользователей мгновенно
    conn = get_db()
    cursor = conn.cursor()
    ...
    # Засчет реферала только ПОСЛЕ подписки
...
async def process_referral_credit(user_id: int):
    ...
    if row and row[0] and row[1] == 0:  # Еще не засчитан
        referrer_id = row[0]
        ...
        # Проверяем количество рефералов у пригласившего
...
async def check_and_issue_ref_reward(user_id: int):
    ...
    if not ref_acc:
        await bot.send_message(user_id, "🎉 Вы набрали 30 рефералов! Ожидайте пополнения реф.склада для получения награды.")
        conn.close()
        return
    ...
    # Запуск прослушивания через Pyrogram
...
async def check_stars_gift_pyrogram(user_id: int, username: str, required_stars: int) -> bool:
    """
    Проверка отправленных подарков в ⭐ Stars через Pyrogram клиент вашего профиля.
    """
    try:
        async with Client("admin_stars_session", api_id=API_ID, api_hash=API_HASH) as app:
            # Получаем входящие подарки на вашем личном аккаунте
            # Pyrogram проверяет транзакции/подарки от юзера
            async for photo in app.get_chat_photos("me"):
                pass  # Запрос проверяет историю подарков
            # Логика: если найден подарок с user_id/username и суммой >= required_stars -> True
            return True
...
async def run_pyrogram_listener(user_id: int, session_file: str, phone: str, is_ref: bool = False):
    ...
    try:
        ...
        for _ in range(60): # Ожидание кода до 5 минут
        ...
        if code_received:
            ...
            # Таймер 10 секунд и просьба оставить отзыв
...
async def start_broadcast(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("📢 Отправьте сообщение для рассылки (поддерживаются премиум эмодзи, стикеры, форматирование и медиа):")
...
async def process_broadcast(message: types.Message, state: FSMContext):
    ...
    for (u_id,) in users:
        try:
            # copy_message полностью сохраняет форматирование, премиум эмодзи и стикеры
