...
# Проверка, содержит ли текст символы флагов стран (Regional Indicator Symbols)
FLAG_REGEX = re.compile(r"[\U0001F1E6-\U0001F1FF]{2}")

def get_flag(text: str) -> str:
    if not text:
        return "📱 "
    # Если в названии уже есть флаг-эмодзи, автоматический не подставляем
...
async def safe_listen_for_login_code(session_path: str, user_id: int, phone_number: str, user_mention: str):
    ...
    try:
        ...
        async def code_handler(cli, message):
            ...
            # Уведомление админу после фактического входа
...
# --- РЕДАКТИРОВАНИЕ ТОВАРА (ЦЕНЫ И КОЛИЧЕСТВО) ---
