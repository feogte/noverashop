...
async def process_payment_proof(m: Message, state: FSMContext):
    ...
    await m.answer("⏳ **Заявка отправлена администратору на проверку.** Ожидайте подтверждения.")
...
async def approve_purchase(call: CallbackQuery):
    ...
    await bot.send_message(purch["user_id"], "✅ **Ваш платеж успешно подтвержден!** Нажмите кнопку ниже, чтобы забрать товар:", reply_markup=claim_kb)
...
# --- ВЫДАЧА И PYROGRAM СЛУШАТЕЛЬ С ТАЙМЕРАМИ ---
            ...
            if message.text:
                ...
                if codes:
                    ...
                    try:
                        ...
                        # Запуск таймеров сообщений после отправки кода
