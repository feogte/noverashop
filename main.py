...
# =====================================================================
TOKEN = os.getenv("BOT_TOKEN") or os.getenv("TELEGRAM_BOT_TOKEN") or "ВАШ_ТОКЕН_БОТА"
...
async def back_from_state(m,state):
    if not await subscription_gate(m): return
    await state.clear(); await m.answer("Главное меню",reply_markup=home_kb(m.from_user.id))
# Автовыдача кода авторизации через Pyrogram
...
async def purchase_action(call):
    ...
    if action=="approve":
        ...
        # Получаем данные товара для автовыдачи
...
async def add_kind(call,state):
    if not await subscription_gate(call): return
    await state.update_data(kind=call.data.split(":")[1]); await state.set_state(AddProduct.name); await call.message.answer("Введите название товара. Для аккаунта: +1 или +66 5 лет"); await call.answer()
...
async def add_stock(m,state):
    ...
    if d["kind"] == "account":
        ...
        await m.answer("Отправьте файл сессии (.session) для этого аккаунта.")
    else:
        conn=await db(); await conn.execute("INSERT INTO products(kind,name,price_rub,price_stars,stock,sales,session_file,phone_number,created_at) VALUES(?,?,?,?,?,0,NULL,NULL,?)",(d["kind"],d["name"],d["price_rub"],d["price_stars"],count,now_msk().isoformat())); await conn.commit(); await conn.close(); await state.clear(); await m.answer("Товар добавлен на склад.",reply_markup=admin_kb())
...
async def add_phone(m, state):
    ...
    d = await state.get_data()
    conn = await db()
    await conn.execute("INSERT INTO products(kind,name,price_rub,price_stars,stock,sales,session_file,phone_number,created_at) VALUES(?,?,?,?,?,0,?,?,?)", (d["kind"], d["name"], d["price_rub"], d["price_stars"], d["stock"], d["session_file"], phone, now_msk().isoformat()))
    await conn.commit(); await conn.close()
    await state.clear()
