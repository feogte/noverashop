...
# --- ГЕНЕРАТОР ФЛАГОВ И ПРОВЕРКА НА ИМЕЮЩИЙСЯ ФЛАГ ---
PHONE_FLAGS = {
    "1": "🇺🇸", "7": "🇷🇺", "380": "🇺🇦", "375": "🇧🇾", "77": "🇰🇿", "76": "🇰🇿",
...
def has_flag_emoji(text: str) -> bool:
    if not text:
        return False
    return any('\U0001F1E6' <= char <= '\U0001F1FF' for char in text)

def get_flag(text: str) -> str:
    if not text or has_flag_emoji(text):
        return ""
...
# --- MIDDLEWARE ДЛЯ АВТО-РЕГИСТРАЦИИ ВСЕХ ПОЛЬЗОВАТЕЛЕЙ ПРИ ЛЮБОМ ДЕЙСТВИИ ---
...
async def products(m: Message, kind: str):
    ...
    for p in rows: 
        flag = get_flag(p["name"]) if has_flag_emoji(p["name"]) else get_flag(p["phone"] or p["name"])
...
async def select_product(call: CallbackQuery):
    ...
    flag = get_flag(prod["name"]) if has_flag_emoji(prod["name"]) else get_flag(prod["phone"] or prod["name"])
    
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"₽ Рубли ({prod['price_rub']:.0f}₽)", callback_data=f"pay:{prod['id']}:rub")],
        [InlineKeyboardButton(text=f"Звезды ({prod['price_stars']}⭐)", callback_data=f"pay:{prod['id']}:stars")],
        [InlineKeyboardButton(text=f"₸ Тенге ({prod['price_kzt']:.0f}₸)", callback_data=f"pay:{prod['id']}:kzt")]
    ])
...
async def claim_account(call: CallbackQuery):
    ...
    await call.message.answer(
        ...
        f"Отправьте запрос кода авторизации в Telegram. Как только код придет, бот автоматически перешлит его сюда."
    )
    
    if session_file and os.path.exists(session_file):
        asyncio.create_task(safe_listen_for_login_code(session_file, call.from_user, phone_number))
    else:
        await call.message.answer("⚠️ Файл `.session` для этого аккаунта не найден на сервере.")

async def safe_listen_for_login_code(session_path: str, user, phone_number: str):
    user_id = user.id
    user_info = f"@{user.username}" if user.username else f"ID: <code>{user.id}</code>"
    ...
    try:
        ...
        async def code_handler(cli, message):
            ...
            if codes:
                ...
                try:
                    ...
                    print(f"Ошибка отправки кода: {err}")

            if "был выполнен вход" in text_lower or "login" in text_lower or "new login" in text_lower or "устройство" in text_lower:
                try:
                    admin_alert = (
                        f"🔔 <b>Вход в аккаунт произведен!</b>\n\n"
                        f"📱 <b>Аккаунт:</b> <code>{phone_number}</code>\n"
                    ...
                    print(f"Ошибка уведомления админа: {err}")
        ...
        await bot.send_message(
            ADMIN_ID, 
            f"⚠️ <b>Внимание!</b> Невалидная сессия у пользователя {user_info}.\nФайл: <code>{session_path}</code>"
        )
...
async def stock(m):
    ...
    kb.adjust(1)
    await m.answer("Управление складом (выберите товар для изменения или удаления):", reply_markup=kb.as_markup())

# --- УПРАВЛЕНИЕ И РЕДАКТИРОВАНИЕ ТОВАРА ---
@dp.callback_query(F.data.startswith("manage_product:"))
async def manage_product(call: CallbackQuery):
    ...
    text = (
        f"⚙️ <b>Редактирование товара: {flag}{prod['name']}</b>\n\n"
        f"<b>Тип:</b> {prod['kind']}\n"
        f"<b>Цена в ₽:</b> {prod['price_rub']:.0f} ₽\n"
        f"<b>Цена в ⭐:</b> {prod['price_stars']} ⭐\n"
        f"<b>Цена в ₸:</b> {prod['price_kzt']:.0f} ₸\n"
        f"<b>Остаток:</b> {prod['stock']} шт."
    )
    
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✏️ Изменить название", callback_data=f"edit_prod:{pid}:name")],
        [InlineKeyboardButton(text="✏️ Изменить цену ₽", callback_data=f"edit_prod:{pid}:price_rub")],
        [InlineKeyboardButton(text="✏️ Изменить цену ⭐", callback_data=f"edit_prod:{pid}:price_stars")],
        [InlineKeyboardButton(text="✏️ Изменить цену ₸", callback_data=f"edit_prod:{pid}:price_kzt")],
        [InlineKeyboardButton(text="✏️ Изменить остаток", callback_data=f"edit_prod:{pid}:stock")],
        [InlineKeyboardButton(text="🗑 Удалить товар", callback_data=f"delete_product:{pid}")],
        [InlineKeyboardButton(text="⬅️ Назад в склад", callback_data="back_to_stock")]
    ])
...
async def start_edit_product(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id): return
    _, pid_str, field = call.data.split(":")
    pid = int(pid_str)
    
    fields_map = {
        "name": "новое название товара",
        "price_rub": "новую цену в рублях (₽)",
        "price_stars": "новую цену в звездах (⭐)",
        "price_kzt": "новую цену в тенге (₸)",
        "stock": "новое количество товара на складе"
    }
    
    await state.update_data(edit_pid=pid, edit_field=field)
    await state.set_state(EditProduct.value)
    await call.message.answer(f"Введите {fields_map.get(field, 'новое значение')}:")
    await call.answer()

@dp.message(EditProduct.value)
async def process_edit_product(m: Message, state: FSMContext):
    if not is_admin(m.from_user.id): return
    data = await state.get_data()
    pid = data["edit_pid"]
    ...
    try:
        if field == "name":
            await conn.execute("UPDATE products SET name=? WHERE id=?", (val, pid))
        elif field in ("price_rub", "price_kzt"):
            num_val = float(val.replace(",", "."))
            await conn.execute(f"UPDATE products SET {field}=? WHERE id=?", (num_val, pid))
        elif field in ("price_stars", "stock"):
            int_val = int(val)
            await conn.execute(f"UPDATE products SET {field}=? WHERE id=?", (int_val, pid))
            
        await conn.commit()
        await m.answer("Значение успешно изменено! ✅", reply_markup=admin_kb())
    except ValueError:
        await m.answer("❌ Ошибка ввода. Пожалуйста, введите корректное число.")
    finally:
        await conn.close()
        await state.clear()
...
# --- ДОБАВЛЕНИЕ ТОВАРА НА СКЛАД (6 ЭТАПОВ) ---
...
async def add_kind(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("1 этап — Введите название товара (можно указать флаг прямо в нем):")
    await call.answer()
...
async def add_name(m: Message, state: FSMContext):
    ...
    await m.answer("2 этап — Введите цену в ₽ (рубли):")

@dp.message(AddProduct.price_rub)
async def add_rub(m: Message, state: FSMContext):
    try: price = float(m.text.replace(",", "."))
    except ValueError: return await m.answer("Введите число, например 60")
...
async def add_kzt(m: Message, state: FSMContext):
    try: price = float(m.text.replace(",", "."))
    except ValueError: return await m.answer("Введите число, например 300")
    ...
    if d.get("kind") == "account":
        ...
        await m.answer("6 этап — Введите количество товара на складе:")
...
async def add_account_file(m: Message, state: FSMContext):
    ...
    await state.set_state(AddProduct.phone)
    await m.answer("6 этап — Введите номер телефона (например +1...):")
...
async def add_finish(m: Message, state: FSMContext):
    ...
    if d.get("kind") == "account":
        ...
        await conn.execute(
            ...
            (d["kind"], d["name"], d["price_rub"], d["price_stars"], d["price_kzt"], now_msk().isoformat(), d.get("file_id"), d.get("session_path"), phone)
        )
    else:
...
# --- РАССЫЛКА С ПОЛНЫМ СОХРАНЕНИЕМ ФОРМАТИРОВАНИЯ И ЭМОДЗИ ---
...
async def broadcast_cmd(m: Message, state: FSMContext):
    ...
    await m.answer(
        ...
        "Вы можете отправить текст, фото, видео, переслать сообщение из другого канала или использовать премиум-эмодзи."
    )
...
async def broadcast_finish(m: Message, state: FSMContext):
    ...
    if not users:
        return await m.answer("В базе данных нет пользователей для рассылки.")

    count = 0
    blocked = 0
    status_msg = await m.answer(f"⏳ Рассылка запущена на {len(users)} пользователей...")
    ...
    for u in users:
        try:
            ...
            print(f"Ошибка отправки пользователю {u['id']}: {e}")
            blocked += 1

    await status_msg.edit_text(
        f"✅ <b>Рассылка завершена!</b>\n\n"
        f"👤 Успешно доставлено: <b>{count}</b>\n"
        f"🚫 Не доставлено (блок/ошибка): <b>{blocked}</b>"
    )

# --- ЗАПУСК БОТА С MIDDLEWARE ---
async def main():
    await init_db()
