...
class AddProduct(StatesGroup):
    kind = State(); name = State(); price_rub = State(); price_stars = State()
    session_file = State(); phone_number = State(); stock = State()
...
async def purchase_action(call):
    ...
    if action=="approve":
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🎁 Забрать аккаунт", callback_data=f"claim_acc:{pid}")]
        ])
        try:
            await bot.send_message(
                p["user_id"], 
                f"Ваша заявка #{pid} одобрена!\nПолучить аккаунт можете по кнопке ниже:", 
                reply_markup=kb
            )
        except Exception: pass
    else: 
        await conn.execute("UPDATE purchases SET status='rejected' WHERE id=?",(pid,)); status="Отклонено"
        try: await bot.send_message(p["user_id"],f"Ваша заявка #{pid}: Отклонено.")
        except Exception: pass
...
async def claim_account(call: CallbackQuery):
    ...
    conn = await db()
    purchase = await one(conn, "SELECT * FROM purchases WHERE id=? AND user_id=?", (pid, call.from_user.id))
    
    if not purchase or purchase["status"] != "approved":
        await conn.close()
        return await call.answer("Заявка не найдена или не одобрена.", show_alert=True)
        
    product = await one(conn, "SELECT * FROM products WHERE id=?", (purchase["product_id"],))
    await conn.close()
    
    if not product or not product["session_file"]:
        return await call.answer("Данные аккаунта не найдены.", show_alert=True)
    
    await call.message.edit_text(
        f"📱 <b>Номер аккаунта:</b> <code>{product['phone_number']}</code>\n\n"
        f"Отправьте код на аккаунт, мы скинем его вам."
    )
    await call.answer()
    
    asyncio.create_task(listen_for_telegram_code(call.from_user.id, product["session_file"]))

async def listen_for_telegram_code(user_id: int, session_path: str):
    ...
    try:
        ...
        @app.on_message(filters.me | filters.service | filters.private)
        async def code_handler(client, message):
            if message.from_user and message.from_user.id == 777000:
                codes = re.findall(r'\b\d{5}\b', message.text or "")
                if codes:
                    code = codes[0]
                    await bot.send_message(user_id, f"🔑 <b>Ваш код:</b> <code>{code}</code>")
                    await client.stop()
...
async def add_stars(m,state):
    ...
    if d.get("kind") == "account":
        ...
        await m.answer("Отправьте файл сессии Pyrogram (.session документацией).")
    else:
...
async def add_phone(m, state):
    ...
    await conn.commit(); await conn.close()
    await state.clear()
    await m.answer("Аккаунт успешно добавлен на склад!", reply_markup=admin_kb())
