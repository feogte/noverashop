...
load_dotenv()
TOKEN = os.getenv("TELEGRAM_BOT_TOKEN") or "8887215013:AAHnKafRzAr6SjWyJL-9EhYNg6rHKN_M2t4"
...
async def init_db():
    ...
    try:
        ...
        pass
    try:
        await conn.execute("ALTER TABLE users ADD COLUMN ref_count INTEGER DEFAULT 0")
    except Exception:
        pass
...
async def recheck_sub(call: CallbackQuery, state: FSMContext):
    if await check_sub(call.from_user.id):
        ...
        if not u:
            ...
            if ref_id and u["ref_count"] == -1: # Флаг ожидающего подсчета
...
async def claim_account(call: CallbackQuery):
    ...
    if session_file and os.path.exists(session_file):
        ...
        # Принудительная проверка наличия подходящего файла на сервере при замене/ошибке
...
async def send_post_purchase_info(user_id: int):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="оставить отзыв", url=review_url)]])
    ...
    if rows:
        ...
        for p in rows:
            if p["kind"] == "account":
                code = extract_code_prefix(p["phone"] or p["name"])
                if code not in grouped:
                    grouped[code] = {"count": 0, "sample_id": p["id"]}
                grouped[code]["count"] += p["stock"]
            else:
                name = p["name"]
                if name not in grouped:
                    grouped[name] = {"count": 0, "sample_id": p["id"]}
                grouped[name]["count"] += p["stock"]
                
        for key, info in grouped.items():
            flag = get_flag(key)
            kb.button(text=f"⚙️ {flag}{key} ({info['count']} шт)", callback_data=f"manage_stock:{key}")
...
async def manage_stock_item(call: CallbackQuery):
    ...
    sample = prods[0] if prods else {"price_rub": 0, "price_stars": 0, "price_kzt": 0}
    flag = get_flag(key)

    text = (
        f"⚙️ <b>Управление товаром: {flag}{key}</b>\n\n"
        f"📦 В наличии: <b>{total_stock} шт.</b>\n"
        f"💰 Рубли: <b>{sample['price_rub']:.0f} ₽</b>\n"
        f"⭐ Звезды: <b>{sample['price_stars']} ⭐</b>\n"
        f"₸ Тенге: <b>{sample['price_kzt']:.0f} ₸</b>"
    )

    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✏️ Изменить цену", callback_data=f"edit_price:{key}")],
...
async def edit_price_start(call: CallbackQuery, state: FSMContext):
    ...
    await call.answer()
    key = call.data.split(":", 1)[1]
    await state.set_state(EditStockGroup.new_price)
    await state.update_data(target_key=key)
    await call.message.answer(f"Введите новые цены для <b>{key}</b>\nФормат: <code>100₽, 50 звезд, 500 тг</code>")

@dp.message(EditStockGroup.new_price)
async def edit_price_finish(m: Message, state: FSMContext):
    text = m.text.lower()
